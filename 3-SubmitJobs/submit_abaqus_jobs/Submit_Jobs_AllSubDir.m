...
disp('Select Folder:')
new_Fold = uigetdir;
new_subFolds = strsplit(genpath(new_Fold),';');
new_subFolds = new_subFolds(2:end-1)';
Folders = new_subFolds;
YN='y';
while true
    clc;
    ...
    if strcmpi(YN,'y')
        ...
        if new_Fold == 0 %GETDIR = Cancelled
            ...
            new_subFolds = strsplit(genpath(new_Fold),';');
            new_subFolds = new_subFolds(2:end-1)';
            Folders = [Folders; new_subFolds]; %#ok<AGROW>           
        end
    else
        break
    end
...
uicontrol('Style','pushbutton','Position',[0.50*W 0.60*H 0.20*W 0.05*H],...
           'String', 'Update List','Callback',{@UpdateList,h,all_inp});
...
function SubmitJobs(~,~,h)
    ...
    wb=waitbar(0,'Starting Simulations');
    for i=1:length(InputName)
        waitbar(i/length(InputName),wb,LS.INP(i).folder);
%         figure(h2)
        tic;
