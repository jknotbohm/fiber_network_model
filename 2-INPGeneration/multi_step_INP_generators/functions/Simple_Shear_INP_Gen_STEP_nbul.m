...
function [final_nodes, final_fibers] = Simple_Shear_INP_Gen_STEP_nbul(file_loc,strain, l_fiber)   
load_file=file_loc;
load(load_file,'final_nodes','final_fibers')
% ammendments for shear case (2 or 4 elements / fiber):
...
side_length = max(final_nodes(:,2))-min(final_nodes(:,2)) ;
% side_length_z=round((max(final_nodes(:,4))-min(final_nodes(:,4))),-1) ;
side_length_z = max(final_nodes(:,4))-min(final_nodes(:,4)) ;
% delta = side_length*strain/2;
delta = side_length*strain;
...
%%============== Finding Edges ============================================
edge_x_pos = find(final_nodes(:,2) > (side_length/2 - 10) );
edge_x_neg = find(final_nodes(:,2) < (-1*side_length/2 + 10) );
edge_y_pos = find(final_nodes(:,3) > (side_length/2 - 120) );
edge_y_neg = find(final_nodes(:,3) < (-1*side_length/2 + 10) );
...
delta = delta / 20 ; % ////displacement in each step applied////
% --- STEP 1 to 20 ---
for ik = 1:1:20
fprintf(fid,'*STEP,NAME=STEP00%.3d, NLGEOM=YES, INC=30000\n',ik);
...
%     fprintf(fid,'XNEG, 1, , %8.4f\n', -delta/2);
    ...
    fprintf(fid,'*BOUNDARY, TYPE=DISPLACEMENT\n');
    fprintf(fid,'YPOS, 1, , %8.4f\n', delta * ik);   % shear (simple)
    fprintf(fid,'*BOUNDARY, TYPE=DISPLACEMENT\n');
%   fprintf(fid,'YNEG, 2, , %8.4f\n', -delta);
    fprintf(fid,'YNEG, 1, , %8.4f\n', 0);   % shear (simple) / bottom edge fixed
    fprintf(fid,'*BOUNDARY, TYPE=DISPLACEMENT\n');
    fprintf(fid,'YNEG, 2, , %8.4f\n', 0);
    fprintf(fid,'*BOUNDARY, TYPE=DISPLACEMENT\n');
    fprintf(fid,'YNEG, 3, , %8.4f\n', 0);
