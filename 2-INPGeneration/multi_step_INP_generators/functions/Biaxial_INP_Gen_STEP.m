...
function [final_nodes, final_fibers] = Biaxial_INP_Gen_STEP(file_loc,strain, l_fiber)   
load_file=file_loc;
...
side_lengthz=round((max(final_nodes(:,4))-min(final_nodes(:,4)))) ;

delta_x = side_lengthx * strain  ;
delta_y = side_lengthy * strain  ;
delta_z = side_lengthz * strain / 2 ;
...
%%============== Finding Edges ============================================
edge_x_pos = find(final_nodes(:,2) > (side_lengthx/2 - 60) );
edge_x_neg = find(final_nodes(:,2) < (-1*side_lengthx/2 + 60) );
edge_y_pos = find(final_nodes(:,3) > (side_lengthy/2 - 60) );
edge_y_neg = find(final_nodes(:,3) < (-1*side_lengthy/2 + 60) );

edge_z_pos = find(final_nodes(:,4) > (side_lengthz/2 - 2.25) );
edge_z_neg = find(final_nodes(:,4) < (-1*side_lengthz/2 + 2.25) );
...
delta_x = delta_x / 20 ; % Twenty equal load steps
delta_y = delta_y / 20 ; % Twenty equal load steps
delta_z = delta_z / 20 ; % Twenty equal load steps

% --- STEP 1-20 ---
for ik = 1:1:20
    
fprintf(fid,'*STEP,NAME=STEP00%.3d, NLGEOM=YES, INC=100000\n',ik);
