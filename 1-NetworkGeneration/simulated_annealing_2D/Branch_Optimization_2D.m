...
function [ nodes, fibers, nodal_branching_energy, total_branching_energy_init, total_branching_energy_final ] = Branch_Optimization_2D( N_branching_optimize, nodes, fibers, N );
...
for j=1:N
    ...
    unit_vecs{j} = zeros(n_cf,2);
    cf_lengths = zeros(n_cf,1);
    for k=1:n_cf
        cf_lengths(k) = ((nodes(othernodes_jth(k),1)-nodes(node1,1))^2 + (nodes(othernodes_jth(k),2)-nodes(node1,2))^2 )^.5;
...
%         end
        ...
        l_endpt = sqrt((endpt(1))^2 + (endpt(2))^2) ;
        ...
        other_spatial_steps = zeros(n_cf,2);
        ...
        for k=1:(n_cf-2)
            ...
            other_spatial_steps(k,:) = stepsize*endpt + (stepsize/50)*(-.5+rand(1,2));
        end
        ...
        cf_lengths = zeros(n_cf,1);
        new_unit_vecs = zeros(n_cf,2);
        
        for k=1:n_cf
            cf_lengths(k) = ((other_new_coords(k,1)-newx)^2 + (other_new_coords(k,2)-newy)^2 )^.5;
...
%            new_unit_vecs(k,3) = (other_new_coords(k,3) - newz)/cf_lengths(k);
        ...
        for l=1:length(othernodes_jth)
            ...
            new_all_unit_vecs{l}=zeros(n_cf,2);
            cf_lengths = zeros(n_cf,1);
         ...
         %   newz_1 = other_new_coords(l,3);
            ...
            for k=1:n_cf
                if k==jth_node_index  % the jth node needs to use the new coordinates, not the ones currently in nodes.
                    cf_lengths(k) = ((newx-newx_1)^2 + (newy-newy_1)^2 )^.5;
           ...
           %         new_all_unit_vecs{l}(k,3) = (newz - newz_1)/cf_lengths(k);
                    ...
                    cf_lengths(k) = ((nodes(othernodes_lth(k),1)-newx_1)^2 + (nodes(othernodes_lth(k),2)-newy_1)^2 )^.5;
                    %unit_vecs(k,:) = (1/cf_lengths(k))*(nodes(othernodes_lth(k,:))-other_new_coords(l,:));
        ...
        if sum(new_nodal_branching_energies) < sum(old_nodal_branching_energies)
            ...
            nodes(cnis,:) = [newx newy ; other_new_coords];
