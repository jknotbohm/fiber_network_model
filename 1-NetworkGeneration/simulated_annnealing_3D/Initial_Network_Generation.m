function [ nodes, fibers, fiberenergy, total_energy, N1, N2, N_boundary_nodes,fiberlengths,valencycheck ] = Initial_Network_Generation(rho,lx,ly,lz,l_fiber,n_neighbors_to_try)
...
N = round(rho*lx*ly*lz);    % Number of nodes
...
nodes_boundary = zeros(N_boundary_nodes,3);

% X boundary:

xbnd1_x = (-lx/2)*ones(NBx,1);
xbnd1_y = (-ly/2) + ly*rand(NBx,1); 
xbnd1_z = (-lz/2) + lz*rand(NBx,1); 

xbnd2_x = (lx/2)*ones(NBx,1);
xbnd2_y = (-ly/2) + ly*rand(NBx,1); 
xbnd2_z = (-lz/2) + lz*rand(NBx,1); 

nodes_boundary(1:2*NBx,:) = [xbnd1_x xbnd1_y xbnd1_z; xbnd2_x xbnd2_y xbnd2_z];

ybnd1_x = (-lx/2) + lx*rand(NBy,1);
ybnd1_y = (-ly/2)*ones(NBy,1);
ybnd1_z = (-lz/2) + lz*rand(NBy,1);

ybnd2_x = (-lx/2) + lx*rand(NBy,1);
ybnd2_y = (ly/2)*ones(NBy,1);
ybnd2_z = (-lz/2) + lz*rand(NBy,1);

nodes_boundary(2*NBx+1:2*NBx+2*NBy,:) = [ybnd1_x ybnd1_y ybnd1_z; ybnd2_x ybnd2_y ybnd2_z];

zbnd1_x = (-lx/2) + lx*rand(NBz,1);
zbnd1_y = (-ly/2) + ly*rand(NBz,1);
zbnd1_z = (-lz/2)*ones(NBz,1);

zbnd2_x = (-lx/2) + lx*rand(NBz,1);
zbnd2_y = (-ly/2) + ly*rand(NBz,1);
zbnd2_z = (lz/2)*ones(NBz,1);

nodes_boundary(2*NBx+2*NBy+1:N_boundary_nodes,:) = [zbnd1_x zbnd1_y zbnd1_z; zbnd2_x zbnd2_y zbnd2_z];

%% Preallocate node array:
nodes = zeros(N,3); %(nodeindex, X, Y, Z)
...
nodes(N_boundary_nodes+1:N,:) = ...
    rand(length(nodes)-N_boundary_nodes,3).*[lx ly lz] - 0.5*[lx ly lz];
...
for k=1:N
    ...
    while length(neighbors) < n_neighbors_to_try+1
        ...
        z_lim = find((nodes(:,1)>node_loc(1)-node_dist) & (nodes(:,1)<node_loc(1)+node_dist));
        neighbors=intersect(intersect(x_lim,y_lim),z_lim);
        
    end
    
    distances = ((nodes(k,1) - nodes(neighbors,1)).^2 + (nodes(k,2) - nodes(neighbors,2)).^2 + (nodes(k,3) - nodes(neighbors,3)).^2).^.5;
    [~, dis] = sort(distances);
...
for k=1:length(val_1)
    % Need to add two more fibers to each of the val_1 nodes
    distances = ((nodes(val_1(k),1) - nodes(:,1)).^2 + (nodes(val_1(k),2) - nodes(:,2)).^2 + (nodes(val_1(k),3) - nodes(:,3)).^2).^.5;
    [~, dis] = sort(distances);
...
for k=1:length(val_2)
    % Need to add two more fibers to each of the val_1 nodes
    distances = ((nodes(val_2(k),1) - nodes(:,1)).^2 + (nodes(val_2(k),2) - nodes(:,2)).^2 + (nodes(val_2(k),3) - nodes(:,3)).^2).^.5;
    [~, dis] = sort(distances);
...
fiberlengths = ((N1(:,1)-N2(:,1)).^2 + (N1(:,2)-N2(:,2)).^2 + (N1(:,3)-N2(:,3)).^2).^.5;
