...
function [ nodes, fibers, fiberenergy, fiberlengths, total_energy, N1,N2,N_interior ] = Network_Optimization( fraction_to_try_swap,N, nodes, fibers, N_anneal, lx,ly,lz,l_fiber,fiberlengths,fiberenergy,N1,N2,N_boundary_nodes, stepsize,swap_skip_energy )
...
for m=1:N_anneal
    ...
    if fork > fraction_to_try_swap
        ...
        for j=N_interior'
            ...
            newz = nodes(j,3) + (lz/lx)*stepsize*(2*(-.5 + rand))*(((N_anneal*N_int)-n_accepted)/(N_anneal*N_int));
            ...
            for k=1:length(nfl)
                nfl(k) =  ((nodes(othernodes(k),1)-newx)^2 + (nodes(othernodes(k),2)-newy)^2 + (nodes(othernodes(k),3)-newz)^2)^.5;
            end
            ...
            if nfe_sum < current_fes_sum
                ...
                fiberlengths(n1f) = ((N1(n1f,1)-N2(n1f,1)).^2 + (N1(n1f,2)-N2(n1f,2)).^2 + (N1(n1f,3)-N2(n1f,3)).^2).^.5;
...
%             [n1f,~] = find(fibers==node1);  % fibers that use node1
            ...
            distances_1 = ((nodes(node1,1) - nodes(:,1)).^2 + (nodes(node1,2) - nodes(:,2)).^2 + (nodes(node1,3) - nodes(:,3)).^2).^.5;
...
%                 end
                ...
                newfiber1length = ((nodes(node1,1)-nodes(node4,1))^2 + (nodes(node1,2)-nodes(node4,2))^2 + (nodes(node1,3)-nodes(node4,3))^2)^.5;
                newfiber2length = ((nodes(node2,1)-nodes(node3,1))^2 + (nodes(node2,2)-nodes(node3,2))^2 + (nodes(node2,3)-nodes(node3,3))^2)^.5;
